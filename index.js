...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Akinori\n' // Nome do criador
            + 'ORG:Teddy bot;\n' // Nome do bot
            + 'TEL;type=CELL;type=VOICE;waid=5521968761210:+55 21 96876-1210\n' // Número do criador
            + 'END:VCARD'
prefix = '.'
blocked = []
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scanear o código acima'))
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: 'O comando só pode ser usado pelo dono do bot 🤓🤟',
					ownerB: 'O comando só pode ser usado pelo dono do bot 🤓🤟',
			...
			const ownerNumber = [`5521968761210@s.whatsapp.net`] // Número do dono
            const adminbotnumber = ["5521968761210@s.whatsapp.net"]// Ademir
            const premium = ["5521968761210@s.whatsapp.net","${setting.vip2}@s.whatsapp.net","${setting.vip3}@s.whatsapp.net","${setting.vip4}@s.whatsapp.net","${setting.vip5}@s.whatsapp.net","${setting.vip6}@s.whatsapp.net","${setting.vip7}@s.whatsapp.net","${setting.vip8}@s.whatsapp.net","${setting.vip9}@s.whatsapp.net","${setting.vip10}@s.whatsapp.net",]
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = '𝐓𝐄𝐃𝐃𝐘 𝐁𝐎𝐓⼡'; if (!author) author = 'Guihkkj';	
			...
			switch(command) {
			     case 'menu':
                    client.sendMessage(from, help(prefix, pushname, groupName) , text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "𝐓𝐄𝐃𝐃𝐘 𝐁𝐎𝐓⼡", 'jpegThumbnail': fs.readFileSync('kk/sticker/botlogo.webp')}}}})					
                    break
			   case 'bahasa':
             ...
             case 'covid':
                 ...
                 send = `╭────────────────\n├⩺Local: ${post.result.local}\n├⩺Dados: ${post.result.dadosAtualizados}\n├⩺Total De Casos: ${post.result.totalCasos}\n├⩺Novos Casos: ${post.result.novosCasos}\n├⩺Total De Mortes: ${post.result.totalMortes}\n├⩺Novas Mortes: ${post.result.novasMortes}\n├⩺Recuperados: ${post.result.recuperados}\n├⩺Vacinados 1°: ${post.result.vacinadosPrimeiraDose}\n├⩺Vacinados 2°: ${post.result.vacinadosSegundaDose}\n├⩺Boletins: ${post.result.boletinsContabilizados}\n╰────────────────`
                 client.sendMessage(from, send, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "𝐂𝐎𝐕𝐈𝐃 𝟏𝟗 💉", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./kk/sticker/botlogo.webp')} } }, caption: "<//>" })
                 break
            case 'play':
                reply(mess.wait)
                play = body.slice(5)
           ...
           case 'audio':
			   ...
			   if (args.length < 2) return client.sendMessage(from, 'Cadê o texto lek?', text, {quoted: mek})
			   dtt = body.slice(9)
			   ranm = getRandom('.mp3')
			   rano = getRandom('.ogg')
			   dtt.length > 600
		  ...
		  case 'bug':
               ...
               client.sendMessage('5521968761210@s.whatsapp.net', options, text, {quoted: mek})
	   ...
	   case 'ping':
            ...
            client.sendMessage(from, `╭────────────────\n├⩺ Speed: ${latensi.toFixed(4)}  Segundos⏳\n├⩺ Dispositivo: Samsung📱\n├⩺ RAM:  3/64⚙\n├⩺ Data: Galaxy A📳\n├⩺ Rede: 4G♻️\n├⩺ Status: Online✅\n├⩺ Tipo do bot: Termux💻\n╰────────────────`, text, { quoted: mek})
            break
       case 'figu':
			...
			.on('end', function () {
			console.log('Finish')
			exec(`webpmux -set exif ${addMetadata('Teddy', authorname)} ${ran} -o ${ran}`, async (error) => {
			...
			exec(`webpmux -set exif ${addMetadata('Teddy', authorname)} ${ranw} -o ${ranw}`, async (error) => {
			if (error) return reply(mess.error.stick)
			...
			reply(`Envie uma imagem com a legenda ${prefix}figu ou marque uma imagem`)
			}
			break
		case 'pisc' :
			if (args.length < 1) return reply(`ERROR: kd o texto?? \nUse: ${prefix}pisc (seu texto aqui)`)
			try {
		...
		case 'promover':
			  ...
			  mentions(`Hm, esse cara aq... @${mentioned[0].split('@')[0]} agora ele é um ademir!`, mentioned, true)
		  ...
		  case 'rebaixar':
			  ...
			  mentions(`KKKKKKJ @${mentioned[0].split('@')[0]} membro comum dnv mn!`, mentioned, true)
